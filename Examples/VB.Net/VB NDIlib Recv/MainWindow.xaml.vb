...
Public Partial Class MainWindow
	...
	Private Sub InitFind()
        ...
        Dim findDesc As New NDIlib.find_create_t() With { _
            .p_groups = groupsPtr, _
            .show_local_sources = True, _
            .p_extra_ips = extraIpsPtr _
        }
	...
	Private Sub Connect(sourceName As [String])
		...
		If _recvInstancePtr <> IntPtr.Zero Then
            ...
            _receiveThread = New Thread(AddressOf ReceiveThreadProc) With { _
                .IsBackground = True, _
                .Name = "NdiExampleReceiveThread" _
            }
	...
	Private Sub SetTallyIndicators(onProgram As Boolean, onPreview As Boolean)
		...
		If _recvInstancePtr <> IntPtr.Zero Then
            ...
            Dim tallyState As New NDIlib.tally_t() With { _
                .on_program = onProgram, _
                .on_preview = onPreview _
            }
            ...
            Select Case NDIlib.recv_capture_v2(_recvInstancePtr, videoFrame, audioFrame, metadataFrame, 1000)
                ...
                Case NDIlib.frame_type_e.frame_type_audio
                    ...
                    If _multiplexProvider Is Nothing OrElse formatChanged Then
                        _multiplexProvider = New MultiplexingWaveProvider(New List(Of IWaveProvider)() From { _
                            _bufferedProvider _
                        }, 2)
                    End If
                    ...
                    Dim interleavedFrame As New NDIlib.audio_frame_interleaved_32f_t() With { _
                        .sample_rate = audioFrame.sample_rate, _
                        .no_channels = audioFrame.no_channels, _
                        .no_samples = audioFrame.no_samples, _
                        .timecode = audioFrame.timecode _
                    }
