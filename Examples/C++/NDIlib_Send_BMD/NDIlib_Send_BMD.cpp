...
struct decklink_ndi_bridge : public IDeckLinkInputCallback
{
	decklink_ndi_bridge(IDeckLinkInput* p_decklink_input, const std::string& ndi_name)
		: m_p_decklink_input(p_decklink_input), m_p_decklink_video_frame(NULL), m_p_ndi_send(NULL),
		  m_frame_rate_n(30000), m_frame_rate_d(1001), m_frame_format(NDIlib_frame_format_type_interleaved)
	...
	{	if (!m_p_decklink_input || !m_p_ndi_send)
		...
		// Flags to use, depending on the input's capatiblities
